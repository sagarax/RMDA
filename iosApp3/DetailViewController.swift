...
class DetailViewController: UIViewController, UIWebViewDelegate, WKNavigationDelegate {
    ...
    lazy var GOISummaries = ["<html><body bgcolor='#DC8CFE'><br/><center><h1>First Summary!</center><br/>\(firstGOISummary)</h1></body></html>","http://chtr.ca/operations/general-rail-operating-instruction/","<html><body bgcolor='#E6E6FA'><br/><center><h1>Third Summary!</h1></center></body></html>","<html><body bgcolor='#F35C3B'><br/><center><h1>Fourth and Last Summary!</h1></center></body></html>"]
    ...
    func getAcknowledgeButton(frame: CGRect, color: UIColor) -> UIButton? {
        let button = UIButton()
        //let button = UIButton(type: .custom)
        button.frame = frame
...
//        button.addTarget(self, action: #selector(DetailViewController.rightButtonAction(_:)), for: .touchUpInside)
    ...
    func loadSummary(wkwebview:WKWebView, pathURLOrText: String){
        ...
        if pathURLOrText.contains("http"){
    ...
    override func viewDidLoad() {
        ...
        do{
            ...
            for url in contents {
                if url.description.contains("iOSApp3-\(filenameOrURL ?? "GOI").pdf") {
...
//                    webview.heightAnchor.constraint(equalTo: pdfview.heightAnchor).isActive = true
                    ...
                    let ackButtonFrame = CGRect(x: 250, y: 700, width: 120, height: 50)
                    if let ackButton = getRightButton(frame: ackButtonFrame, color: .white) {
